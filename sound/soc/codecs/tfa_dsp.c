...
int  CalibrationFailedRetry = 3;
int  ColdStartupSucessful = 0;

extern int isFactoryTest;
...
int tfa98xx_dsp_system_stable(struct tfa98xx *tfa98xx, int *ready)
{
	...
	*ready = 0;
	...
	pr_info("statusreg = 0x%04x, sysctrl=0x%04x\n", status, sysctrl);
...
static int tfa98xx_startup(struct tfa98xx *tfa98xx)
{
    ...
    */
	...
	if (tries == CFSTABLE_TRIES) {
		pr_err("Wait for DSP system stable - Time out!!!\n");
		return -ETIMEDOUT;
	}  else {
		pr_debug("OK (tries=%d)\n", tries);
	}
...
int tfa98xx_coldboot(struct tfa98xx *tfa98xx, int state)
{
	...
	while (state == !tfa98xx_is_coldboot(tfa98xx)) {
		...
		if (tries-- == 0) {
			pr_err("Error:  coldboot (ACS) did not %s\n",
				  state ? "set" : "clear");
			return -EINVAL;
		}
	}
	...
	if (checkaddress != 0xffff) {
		...
		if (!ret) {
			if (!status) {
				...
				ret = -EBUSY;
				pr_err("check_ic_rom: DSP subsys not ready\n");
			}
		}
...
int tfaRunSpeakerBoost(struct tfa98xx *tfa98xx, int force)
{
	...
	if (force || tfa98xx_is_coldboot(tfa98xx)) {
		...
		if (!force) {
			ret = tfa98xx_startup(tfa98xx);
			if (ret) {
				pr_err("Error: tfa98xx_startup %d\n", ret);
				return ret;
			}
		...
		if (!done) {
			if(CalibrationFailedRetry > 0){
				CalibrationFailedRetry --;
				tfa98xx->dsp_init = TFA98XX_DSP_INIT_RECOVER;
			}
			else
				tfa98xx->dsp_init = TFA98XX_DSP_INIT_FAIL;
			pr_err("Calibration not done!   CalibrationFailedRetry=%d\n", CalibrationFailedRetry);
			/*
			 * Don't return on timeout, otherwise we cannot power
			 * down the DSP
			 */
		} else {
			CalibrationFailedRetry = 0;
			ColdStartupSucessful = 1;
...
int tfa98xx_dsp_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	if (forcecoldboot || tfa98xx_is_coldboot(tfa98xx)) {
		...
		if (tfaRunSpeakerBoost(tfa98xx, forcecoldboot)){
			pr_err("cold start up, tfaRunSpeakerBoost error!\n");
			if((ColdStartupSucessful == 0) && (CalibrationFailedRetry > 0))
				tfa98xx->dsp_init = TFA98XX_DSP_INIT_RECOVER;
			return -EINVAL;
		}
	} else {
	...
	if(isFactoryTest)
	{
		pr_err("BYPASS DSP in factory mode\n");
		ret = snd_soc_write(codec, TFA98XX_I2SREG, 0x880b);
		if (ret)
			pr_err("Write 0x04 error In factory mode\n");
		ret = snd_soc_write(codec, TFA98XX_SYS_CTRL, 0x608);
		if (ret)
			pr_err("Write 0x09 error In factory mode\n");
	}
...
int tfa98xx_dsp_stop(struct tfa98xx *tfa98xx)
{
	...
	{
		status = snd_soc_read(codec, TFA98XX_STATUSREG);
		pr_err("tfa98xx_dsp_stop:  mute device error, status reg = %d\n", status);
		return ret;
	}
