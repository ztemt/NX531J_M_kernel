...
static void pwr_supply_enable(struct sii_usbp_policy_engine *pdev,
		bool is_enable)
{
	if (is_enable)
...
{
	if (is_set)
		sii_platform_set_bit8(REG_ADDR__PDCCSRST,
			BIT_MSK__PDCCSRST__REG_PD24_SRST);
	else
		sii_platform_clr_bit8(REG_ADDR__PDCCSRST,
			BIT_MSK__PDCCSRST__REG_PD24_SRST);
}
		...
		bool is_set)
{
	if (is_set)
		sii_platform_set_bit8(REG_ADDR__PDCCSRST,
			BIT_MSK__PDCCSRST__REG_CC24_SRST);
	else
		sii_platform_clr_bit8(REG_ADDR__PDCCSRST,
			BIT_MSK__PDCCSRST__REG_CC24_SRST);
}
...
static void sii70xx_src_caps(struct sii_usbp_policy_engine *pUsbpd,
	enum pdo_type supply_type)
{
	if (supply_type == VARIABLE_SUPPLY) {
		...
		pUsbpd->pd_src_caps[PDO_IDX_5V] = PDO_FIXED(5000, 900,
				PDO_FIXED_FLAGS);
		pUsbpd->pd_src_cap_cnt = PDO_IDX_5V + 1;
	}
}

static void sii70xx_snk_caps(struct sii_usbp_policy_engine *pUsbpd,
	enum pdo_type supply_type)
{
	if (supply_type == VARIABLE_SUPPLY) {
		/*Yet to Define the caps*/
		...
		/*As per power profiles defined in the spec*/
		pUsbpd->pd_snk_caps[PDO_IDX_5V] = PDO_FIXED(5000, 900,
			PDO_FIXED_FLAGS);
		pUsbpd->pd_src_cap_cnt = PDO_IDX_5V + 1;
	}
}
...
void send_request_msg(struct sii_usbp_policy_engine *pUsbpd)
{
	...
	if (pUsbpd->rdo & RDO_CAP_MISMATCH) {
		set_bit(CAN_NOT_BE_MET, &pUsbpd->intf.param.
				sm_cmd_inputs);
		pr_debug(" Mismatch");
		return;
	}
...
bool process_sysfs_commands(uint8_t bus_id,
	enum ctrl_msg ctrl_msg_type,
		uint32_t *msg_data, uint8_t msg_size)
{
	return true;
}
...
{
	if (!sii_platform_rd_reg8(REG_ADDR__PDTXCS)) {

		sii_platform_wr_reg8(REG_ADDR__PDTXBC, 0x60);
		sii_platform_set_bit8(REG_ADDR__PDTXCS,
...
void si_update_pd_status(struct sii_usbp_policy_engine *pUsbpd)
{
	if (pUsbpd->pr_swap.done) {
		if (pUsbpd->evnt_notify_fn)
			pUsbpd->evnt_notify_fn(pUsbpd->drv_context,
				FEAT_PR_SWP);
...
{
	if (sii_platform_rd_reg8(REG_ADDR__PDCTR11)
...
void change_drp_pwr_role(struct sii_usbp_policy_engine *pUsbpd)
{
	if (sii_platform_rd_reg8(REG_ADDR__PDCTR11)
		...
		/*pdev->drv_context->drp_config = USBPD_UFP;*/
...
uint8_t sii_usbpd_get_src_cap(struct sii_usbp_policy_engine *pUsbpd,
		uint32_t *src_pdo, enum pdo_type type_sup)
{
	memcpy(src_pdo, pUsbpd->pd_src_caps,
			(pUsbpd->pd_src_cap_cnt*sizeof(*src_pdo)));
...
static void no_response_timer_handler(void *context)
{
	...
	if (!down_interruptible(&usbpd_dev->drv_context->isr_lock)) {
		if (drv_context->drp_config == USBPD_DFP) {
			...
			} else if (usbpd_dev->hard_reset_counter <=
				N_HARDRESETCOUNT) {
...
bool usbpd_timer_create(struct sii_usbp_policy_engine *pUsbpd)
{
	...
	if (!pUsbpd->usbpd_inst_no_resp_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to register NoResponseTimer timer!\n");
			return false;
		}
	}
...
void process_hard_reset(struct sii70xx_drv_context *drv_context)
{
	...
	if (drv_context->drp_config == USBPD_DFP) {
		...
		usbpd_dev->state = PE_SRC_Startup;
		sii70xx_pd_sm_reset(usbpd_dev);
		sii70xx_vbus_enable(usbpd_dev->drv_context,
			VBUS_SRC);
	} else {
		pr_debug("HR: UFP\n");
...
bool usbpd_delete_timer_func(struct sii_usbp_policy_engine *pdev)
{
	...
	if (!pdev) {
		pr_warning("%s: failed to delete timer.\n", __func__);
		return false;
	}
	...
	if (pdev->usbpd_inst_no_resp_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete inst_no_resp_tmr!\n");
			return false;
		}
	}
	return true;
	...
	bool (*event_notify_fn)(void*, uint32_t))
{
	...
	usbpd_dev = kzalloc(
		sizeof(struct sii_usbp_policy_engine), GFP_KERNEL);

	if (!usbpd_dev) {
		pr_err("usbpd_pf_dev  Error.\n");
		return NULL;
	}
	...
	/*INIT_WORK(&usbpd_dev->pd_dfp_work, source_policy_engine);
	INIT_WORK(&usbpd_dev->pd_ufp_work, sink_policy_engine);
	schedule_work(&usbpd_dev->pd_ufp_work);
	schedule_work(&usbpd_dev->pd_dfp_work);*/

	usbpd_dev->dfp_work_queue =
		create_singlethread_workqueue(SII_DRIVER_NAME);
	if (usbpd_dev->dfp_work_queue == NULL)
		goto exit;
	...
	usbpd_dev->ufp_work_queue =
		create_singlethread_workqueue(SII_DRIVER_NAME);
	if (usbpd_dev->ufp_work_queue == NULL)
		goto exit;
