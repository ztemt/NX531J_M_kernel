...
/**********************************************************************/
int sii_usbpd_src_exit_mode_req(struct sii_usbp_policy_engine *pUsbpd,
	uint8_t portnum)
{
	...
	if (!pUsbpd->at_mode_established) {
		pr_info("\n !!!!!ALT Mode is not established!!!\n");
		return -EINVAL;
	}
	if (pUsbpd->exit_mode_req_send) {
		pr_info("\n !!!!Exit mode in progress!!!!!!!!\n");
		return -EINVAL;
	}
	if (pUsbpd->state == PE_SRC_Ready) {
		...
		queue_work(pUsbpd->dfp_work_queue, &pUsbpd->pd_dfp_work);
		return true;
	} else {
...
int sii_usbpd_src_alt_mode_req(struct sii_usbp_policy_engine *pUsbpd,
	uint8_t portnum)
{
	if (!pUsbpd->pd_connected) {
		pr_info("\n !!!!!PD is not Established!!!!\n");
...
int sii_usbpd_src_pwr_swap(struct sii_usbp_policy_engine *pUsbpd,
	uint8_t portnum)
{
	if (!pUsbpd->pd_connected) {
		pr_info("\n !!!!!PD is not Established!!!!\n");
	...
	if (pUsbpd->pr_swap.req_send ||
		pUsbpd->pr_swap.req_rcv) {
		pr_info("\n pr_swap in progress\n");
		return -EINVAL;
	}
	if (pUsbpd->state == PE_SRC_Ready) {
...
int sii_usbpd_src_data_swap(struct sii_usbp_policy_engine *pUsbpd,
	uint8_t portnum)
{
	if (!pUsbpd->pd_connected) {
		pr_info("\n !!!!!PD is not Established!!!!\n");
...
int sii_usbpd_req_src_vconn_swap(struct sii_usbp_policy_engine *pUsbpd,
	uint8_t portnum)
{
	if (!pUsbpd->pd_connected) {
		pr_info("\n !!!!!PD is not Established!!!!\n");
...
static void sender_response_timer_handler(void *context)
{
	...
	if (!down_interruptible(&pdev->drv_context->isr_lock)) {
		...
		if (pdev->state == PE_SRC_Get_Sink_Cap) {
			...
			if (pdev->hard_reset_counter < N_HARDRESETCOUNT) {
				pdev->state = PE_SRC_Hard_Reset;
				/*commented to synchronize with nuvuton*/
				goto success;
			}
		} else if (pdev->vconn_swap_state ==
...
{
	pr_info("\n$$ BIST_TIMER $$\n");
	sii_platform_clr_bit8(REG_ADDR__PDCTR18,
		BIT_MSK__PDCTR18__RI_BIST_ENABLE);
}
...
{
	struct sii_usbpd_protocol *pd =
		(struct sii_usbpd_protocol *) context;
	struct sii70xx_drv_context *drv_context =
		(struct sii70xx_drv_context *)pd->drv_context;
	struct sii_usbp_policy_engine *pdev =
...
bool usbpd_source_timer_create(struct sii_usbp_policy_engine *pUsbpd)
{
	...
	if (!pUsbpd) {
		pr_warning("%s: failed in timer init.\n", __func__);
		return false;
	}
	if (!pUsbpd->usbpd_inst_sendr_resp_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register SenderResTimer timer!\n");
			return false;
		}
	}
	if (!pUsbpd->usbpd_inst_source_act_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register SourceActTimer timer!\n");
			goto exit;
		}
	}
	if (!pUsbpd->usbpd_inst_ps_source_on_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register PSSourceOnTimer!\n");
			goto exit;
		}
	}
	if (!pUsbpd->usbpd_source_cap_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register SourceCapabilityTimer!\n");
			goto exit;
		}
	}
	if (!pUsbpd->usbpd_inst_tbist_tmr) {
		status = sii_timer_create(bist_timer_handler,
				pUsbpd, &pUsbpd->usbpd_inst_tbist_tmr,
				tBISTCOUNT, false);

		if (status != 0) {
			pr_warning("Failed to register BISTTimer!\n");
			goto exit;
		}
	}
	if (!pUsbpd->usbpd_vdm_mode_entry_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register vdm_mode_entry_timer!\n");
			goto exit;
		}
	}
	if (!pUsbpd->usbpd_vdm_resp_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register vdm_response_timer!\n");
			goto exit;
		}
	}
	if (!pUsbpd->usbpd_vdm_exit_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register vdm_exit_timer!\n");
			goto exit;
		}
	}
	...
	if (!pUsbpd->usbpd_tSnkTransition_tmr) {
		status = sii_timer_create(wait_on_timer_handler,
				pUsbpd, &pUsbpd->usbpd_tSnkTransition_tmr,
				30, false);
...
bool usbpd_source_timer_delete(struct sii_usbp_policy_engine *pUsbpd)
{
	...
	if (pUsbpd->usbpd_tSnkTransition_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete vdm_exit_tmr!\n");
			return false;
		}
	}
	...
	if (pUsbpd->usbpd_inst_vconn_on_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete usbpd_inst_vconn_on_tmr!\n");
			return false;
		}
	}
	...
	if (pUsbpd->usbpd_vdm_resp_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete vdm_resp_tmr!\n");
			return false;
		}
	}
	...
	if (pUsbpd->usbpd_vdm_mode_entry_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete vdm_mode_entry_tmr!\n");
			return false;
		}
	}
	if (pUsbpd->usbpd_source_cap_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete source_cap_tmr\n");
			return false;
		}
	}
	...
	if (pUsbpd->usbpd_inst_source_act_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete inst_source_act_tmr!\n");
			return false;
		}
	}
	if (pUsbpd->usbpd_inst_sendr_resp_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete inst_sendr_resp_tmr!\n");
			return false;
		}
	}
	return true;
...
{
	struct sii70xx_drv_context *drv_context =
		(struct sii70xx_drv_context *)pdev->drv_context;
	...
	case PE_DFP_VDM_Identity_Request:
		...
		pr_info("\n=================================\n");
		usbpd_send_vdm_cmd(pUsbpd_prtlyr, VDM,
				CMD_DISCOVER_IDENT,
				pdev->svid_mode);
	...
	case PE_DFP_VDM_Wait_Identity_ACK_Rcvd:
		if (test_bit(SVDM_DISC_IDEN_ACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs)) {
			sii_timer_stop(&(pdev->usbpd_vdm_resp_tmr));
			clear_bit(SVDM_DISC_IDEN_ACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs);
			pdev->alt_mode_state = PE_DFP_VDM_Identity_ACKed;
			work = 1;
			/*pdev->at_mode_in_progress = true;*/
		} else if (test_bit(SVDM_DISC_IDEN_NACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs)) {
			sii_timer_stop(&(pdev->usbpd_vdm_resp_tmr));
			clear_bit(SVDM_DISC_IDEN_NACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs);
	...
	case DFP_Identity_Request:
		...
		pr_info("\n============================\n");
		usbpd_send_vdm_cmd(pUsbpd_prtlyr, VDM,
				CMD_DISCOVER_SVID, pdev->svid_mode);
	...
	case DFP_SVIDs_Wait_Good_Crc_Rcvd:
		if (pdev->tx_good_crc_received) {
			...
			if (test_bit(DISCOVER_SVID_ACK_RCVD, &pdev->intf.
					param.svdm_sm_inputs)) {
				work = 1;
			}
		}
		break;
	case DFP_DISCOVER_SVID_ACK:
		if (test_bit(DISCOVER_SVID_ACK_RCVD, &pdev->intf.
			...
			sii_timer_stop(&(pdev->
				usbpd_vdm_resp_tmr));
			clear_bit(DISCOVER_SVID_ACK_RCVD,
					&pdev->intf.param.svdm_sm_inputs);
			pdev->alt_mode_state = DFP_SVIDs_Request;
			work = 1;
			if (test_bit(DISPLAY_PORT_RCVD, &pdev->intf.
					param.svdm_sm_inputs)) {
				clear_bit(DISPLAY_PORT_RCVD,
					&pdev->intf.param.svdm_sm_inputs);
				pdev->at_mode_established = false;
				pdev->alt_mode_req_rcv = false;
				pdev->alt_mode_req_send = false;
		...
		} else if (test_bit(DISCOVER_SVID_NACK_RCVD, &pdev->intf.
					param.svdm_sm_inputs)) {
				clear_bit(DISCOVER_SVID_NACK_RCVD,
					&pdev->intf.param.svdm_sm_inputs);
				pdev->at_mode_established = false;
				pdev->alt_mode_req_rcv = false;
				pdev->alt_mode_req_send = false;
	...
	case DFP_DISCOVER_SVID_MODES_ACK:
		if (test_bit(DISCOVER_MODE_ACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs)) {
			clear_bit(DISCOVER_MODE_ACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs);
			sii_timer_stop(&(pdev->usbpd_vdm_resp_tmr));
			work = 1;
			pdev->alt_mode_state = drp_modes_Request;
		} else if (test_bit(DISCOVER_MODE_NACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs)) {
			clear_bit(DISCOVER_MODE_NACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs);
			sii_timer_stop(&(pdev->usbpd_vdm_resp_tmr));
			pr_info("NACK RECEIVED\n");
			pdev->at_mode_established = false;
	...
	case DFP_SVIDs_Enter_Modes_Good_Crc_Rcvd:
		if (pdev->tx_good_crc_received) {
			...
			sii_timer_start(&(pdev->
				usbpd_vdm_mode_entry_tmr));
		}
		break;
	case DFP_SVIDs_ENTER_MODE_ACK_RCVD:
		if (test_bit(ENTER_MODE_ACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs)) {
			sii_timer_stop(&(pdev->
				usbpd_vdm_mode_entry_tmr));
			pdev->alt_mode_state = drp_mode_Entry_ACKed;
			clear_bit(ENTER_MODE_ACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs);
			work = 1;
		} else if (test_bit(ENTER_MODE_NACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs)) {
			clear_bit(ENTER_MODE_NACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs);
			sii_timer_stop(&(pdev->usbpd_vdm_resp_tmr));
			pr_info("NACK RECEIVED\n");
			pdev->at_mode_established = false;
	...
	case PE_DFP_VDM_Mode_Exit_Request:
		/*DPM*/
		pr_debug("PE_DFP_VDM_Mode_Exit_Request\n");
		usbpd_send_vdm_cmd(pUsbpd_prtlyr, VDM,
				CMD_EXIT_MODE, pdev->svid_mode);
	...
	case PE_drp_mode_Wait_Exit_ACKed:
		if (test_bit(EXIT_MODE_ACK_RCVD, &pdev->intf.param.
					svdm_sm_inputs)) {
			sii_timer_stop(&(pdev->usbpd_vdm_exit_tmr));
			clear_bit(EXIT_MODE_ACK_RCVD, &pdev->intf.param.
				sm_cmd_inputs);
...
bool sii_src_dr_swap_engine(struct sii_usbp_policy_engine *pdev)
{
	...
	case PE_SRC_DR_Swap_Init:
		if (pdev->drv_context->pUsbpd_dp_mngr->
			dr_swap_supp) {
	...
	case PE_SWAP_Wait_Good_Crc_Received:
		if (pdev->tx_good_crc_received) {
			...
			if (test_bit(ACCEPT_RCVD, &pdev->intf.param.
						sm_cmd_inputs)) {
				work = 1;
			} else if (test_bit(REJECT_RCVD, &pdev->intf.param.
					sm_cmd_inputs)) {
				work = 1;
			} else if (test_bit(WAIT_RCVD, &pdev->intf.param.
					sm_cmd_inputs)) {
				work = 1;
			}
		}
	...
	case PE_PRS_Wait_Accept_DR_Swap:
		if (test_bit(ACCEPT_RCVD, &pdev->intf.param.
			...
			clear_bit(ACCEPT_RCVD, &pdev->intf.
					param.sm_cmd_inputs);
			...
			work = 1;
					...
					sm_cmd_inputs)) {
			sii_timer_stop(&(pdev->usbpd_inst_sendr_resp_tmr));
			clear_bit(REJECT_RCVD, &pdev->intf.
					param.sm_cmd_inputs);
...
bool sii_src_vconn_swap_mode_engine(struct sii_usbp_policy_engine *pdev)
{
	...
	case PE_SWAP_Wait_Ps_Rdy_Received:
		...
		if (test_bit(PS_RDY_RCVD, &pdev->intf.param.sm_cmd_inputs)) {
			clear_bit(PS_RDY_RCVD, &pdev->
				intf.param.sm_cmd_inputs);
			sii_timer_stop(&
					(pdev->usbpd_inst_vconn_on_tmr));
...
bool sii_src_pr_swap_engine(struct sii_usbp_policy_engine *pdev)
{
	...
	pdev->dr_swap.done = false;/*clear dr_Swap bits*/
	...
	case PE_SRC_Wait_Src_rdy_Good_Crc_Received:
		if (pdev->tx_good_crc_received) {
			...
			pdev->pr_swap_state = PE_SWAP_Wait_Ps_Rdy_Received;
			if (test_bit(PS_RDY_RCVD, &pdev->intf.param.
				sm_cmd_inputs)) {
				work = 1;
			}
		}
	...
	case PE_SWAP_Wait_Ps_Rdy_Received:
		...
		if (test_bit(PS_RDY_RCVD, &pdev->intf.param.sm_cmd_inputs)) {
			clear_bit(PS_RDY_RCVD, &pdev->
				intf.param.sm_cmd_inputs);
			sii_timer_stop(&
					(pdev->usbpd_inst_ps_source_on_tmr));
			pr_info("\n%s: !!! PR SWAP COMPLETED !!!:%s\n",
...
{
	struct sii70xx_drv_context *drv_context =
		(struct sii70xx_drv_context *)pUsbpd->drv_context;
	struct sii_usbpd_protocol *pUsbpd_protlyr =
		(struct sii_usbpd_protocol *)drv_context->pUsbpd_prot;

	send_custom_vdm_message(pUsbpd_protlyr,
		0xF, pUsbpd->vdm_cnt);
...
void source_policy_engine(struct work_struct *w)
{
	struct sii_usbp_policy_engine *pdev =
		container_of(w, struct sii_usbp_policy_engine,
		pd_dfp_work);
	struct sii70xx_drv_context *drv_context =
		(struct sii70xx_drv_context *)pdev->drv_context;
	struct sii_usbpd_protocol *pUsbpd_protlyr =
	...
	if (!down_interruptible(&drv_context->isr_lock)) {
		pr_info("\n----------------------------------\n");
		pr_info("SOURCE LOCK -  %x\t", pdev->state);
		switch (pdev->state) {
		...
		case PE_SRC_Negotiate_Capability:
			...
			if (test_bit(CAN_NOT_BE_MET, &pdev->intf.
				param.sm_cmd_inputs)) {
		...
		case PE_SRC_Transition_Supply:
			...
			if (pdev->drv_context->pUsbpd_dp_mngr->go_to_min) {
				...
				pr_info("\n========================\n");
				result = usbpd_xmit_ctrl_msg(pUsbpd_protlyr,
						CTRL_MSG__GO_TO_MIN);
			} else {
				pr_info("\n======================\n");
		...
		case PE_SRC_Wait_Accept_Good_Crc_Received:
			...
			if (pdev->tx_good_crc_received) {
				...
				pr_err("\nGOOD CRC not received -2\n");
				usbpd_xmit_ctrl_msg(pUsbpd_protlyr,
		...
		case PE_SRC_Capability_Response:
			...
			if (test_bit(CAN_NOT_BE_MET, &pdev->intf.
				...
				result = usbpd_xmit_ctrl_msg(pUsbpd_protlyr,
					CTRL_MSG__REJECT);
			} else if (test_bit(LATER_GB, &pdev->intf.param.
					sm_cmd_inputs)) {
				pr_debug("set the bit later\n");
				clear_bit(LATER_GB, &pdev->intf.param.
						sm_cmd_inputs);
				pr_info("\n====================\n");
		...
		case PE_SRC_Ready:
			pr_info("!!! ... SRC READY... !!!\n");
			if (test_bit(GOTOMIN_RCVD,
				&pdev->intf.param.sm_cmd_inputs)) {
				clear_bit(GOTOMIN_RCVD, &pdev->intf.
					param.sm_cmd_inputs);
				msleep(100);
			...
			if (test_bit(BIST_RCVD,
				&pdev->intf.param.sm_cmd_inputs)) {
				clear_bit(BIST_RCVD, &pdev->intf.
					param.sm_cmd_inputs);
				drv_context->irq_disable = true;
			}

			if (test_bit(SOFT_RESET_RCVD,
					&pdev->intf.
					param.sm_cmd_inputs)) {
				clear_bit(SOFT_RESET_RCVD,
					&pdev->intf.param.sm_cmd_inputs);
				usbpd_xmit_ctrl_msg(pUsbpd_protlyr,
					CTRL_MSG__ACCEPT);
			}
			...
			if (test_bit(PR_SWAP_RCVD, &pdev->
				intf.param.sm_cmd_inputs)) {
				clear_bit(PR_SWAP_RCVD, &pdev->
					intf.param.sm_cmd_inputs);
			...
			if (test_bit(SVDM_DISC_IDEN_ACK_RCVD, &pdev->
				intf.param.svdm_sm_inputs)) {
			...
			if (test_bit(DR_SWAP_RCVD, &pdev->intf.
				param.sm_cmd_inputs)) {
				clear_bit(DR_SWAP_RCVD, &pdev->intf.
					param.sm_cmd_inputs);
				pdev->dr_swap_state =
			...
			if (test_bit(GET_SINK_CAP_RCVD, &pdev->
				intf.param.sm_cmd_inputs)) {
				clear_bit(GET_SINK_CAP_RCVD, &pdev->
					intf.param.sm_cmd_inputs);
				if (pdev->drv_context->pUsbpd_dp_mngr->
					get_sink_cap) {
					work = 1;
			...
			if (test_bit(GET_SOURCE_CAP_RCVD, &pdev->
				intf.param.sm_cmd_inputs)) {
				/*pr_debug("get source cap received\n");*/
				clear_bit(GET_SOURCE_CAP_RCVD, &pdev->
					intf.param.sm_cmd_inputs);
				pdo_value = sii_usbpd_get_src_cap(pdev,
					pUsbpd_protlyr->send_msg, FIXED_SUPPLY);
				pr_info("\n=======================\n");
		...
		case PE_SRC_Give_Source_Cap:
			...
			pr_info("\n=======================\n");
			result = sii_usbpd_xmit_data_msg(pUsbpd_protlyr,
				SRCCAP, pUsbpd_protlyr->send_msg, pdo_value);
			pdev->next_state =
				PE_SRC_Wait_Src_rdy_Good_Crc_Received;
			break;
	...
	if (work) {
		work = 0;
		queue_work(pdev->dfp_work_queue, &pdev->pd_dfp_work);
	}
	pr_debug("\nSRC QUEUE COMPLETED\n");
}
...
void wakeup_dfp_queue(struct sii70xx_drv_context *drv_context)
{
	struct sii_usbp_policy_engine *pUsbpd =
		(struct sii_usbp_policy_engine *)drv_context->
		pusbpd_policy;
...
bool usbpd_set_dfp_swap_init(struct sii_usbp_policy_engine *pUsbpd)
{
	bool ret = true;
	if (!pUsbpd) {
		pr_warning("%s: Error initialisation.\n", __func__);
		return false;
	}
	pr_debug("\n***usbpd_set_dfp_swap_init***\n");
	...
	set_pd_reset(pUsbpd->drv_context,
		false);
...
bool usbpd_set_dfp_init(struct sii_usbp_policy_engine *pUsbpd)
{
	...
	sii_platform_put_bit8(REG_ADDR__PDCTR11,
		BIT_MSK__PDCTR11__RI_PRL_SPEC_REV, 0x01);
