...
#define LEV_SHF		4
#define NUM_GPIO	5
#define VBUS_DEFAULT	6

#define GPIO_USBPD_INT	11
#define GPIO_VBUS_SRC	5
#define GPIO_VBUS_SNK	4
#define GPIO_RESET_CTRL    16
#define GPIO_LEV_SHF		47
...
SiiPlatformDebugAssert(__FILE__, __LINE__, (uint64_t)(expr), NULL),\
	((void)1) : ((void)0)))
...
void sii_platform_put_bit8(uint8_t addr,
	uint8_t mask, uint8_t val);

void sii_platform_fifo_write8(uint8_t addr, const uint8_t *p_data,
	uint16_t size);
void sii_platform_fifo_read8(uint8_t addr, uint8_t *p_data,
	uint16_t size);
void sii_platform_block_write8(uint8_t addr, const uint8_t *p_data,
	uint16_t size);
void sii_platform_block_read8(uint8_t addr, uint8_t *p_data,
	uint16_t size);
...
bool process_sysfs_commands(uint8_t bus_id, enum ctrl_msg type,
...
struct sii_typec {
	...
	struct workqueue_struct *typec_work_queue;
...
struct sii_usbp_policy_engine {
	...
	struct		work_struct pd_dfp_work;
	struct		work_struct pd_ufp_work;
	struct		workqueue_struct *dfp_work_queue;
	struct		workqueue_struct *ufp_work_queue;
...
/**************************TYPE C******************************/
void sii_phy_exit(void *ptypec_dev);
void *sii_phy_init(struct sii70xx_drv_context *);
/****************************SYSFS****************************/
#if defined(I2C_DBG_SYSFS)
...
bool sii_drv_sysfs_init(struct sii70xx_drv_context *pdev,
	struct device *dev);
#endif
/*************************Driver*******************************/
...
void sii70xx_pd_reset_variables(struct sii_usbp_policy_engine *pd_dev);
int usbpd_process_rx_data(struct sii70xx_drv_context *drv_context,
...
void update_typec_status(struct sii_typec *ptypec_dev, bool status,
	bool is_dfp);
void set_cc_reset(struct sii70xx_drv_context *drv_context,
		bool is_set);
void set_pd_reset(struct sii70xx_drv_context *drv_context, bool is_set);
...
int sii_drv_set_pr_swap(struct sii70xx_drv_context *drv_context,
	uint8_t portnum);
int sii_usbpd_req_power_swap(struct sii70xx_drv_context *drv_context,
	uint8_t portnum);
...
int sii_usbpd_snk_alt_mode_req(struct sii_usbp_policy_engine *pUsbpd,
	uint8_t portnum);
int sii_usbpd_req_exit_mode(struct sii70xx_drv_context *drv_context,
	uint8_t portnum);
...
int sii_drv_set_dr_swap(struct sii70xx_drv_context *drv_context,
	uint8_t portnum);
int sii_usbpd_req_data_swap(struct sii70xx_drv_context *drv_context,
	uint8_t portnum);
...
int sii_drv_get_sr_cap(struct sii70xx_drv_context *drv_context,
	uint8_t portnum);
int sii_usbpd_req_src_cap(struct sii70xx_drv_context *drv_context,
		uint8_t portnum);
...
int sii_drv_set_vconn_swap(struct sii70xx_drv_context *drv_context,
	uint8_t portnum);
bool sii_usbpd_req_vconn_swap(struct sii70xx_drv_context *drv_context,
	uint8_t portnum);
...
void drp_phy_config(struct sii_typec *phy,
		enum phy_drp_config drp_role);
