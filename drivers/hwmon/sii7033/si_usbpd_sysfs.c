...
ssize_t show_xmit_ctrl(struct device *dev, struct device_attribute *attr,
		char *buf)
{
		...
		const char *buf, size_t count)
{
	...
	for (i = 0; ((i < 8) && ('\0' != *str)); i++) {
		...
		if (-ERANGE == status) {
			...
			pr_info("entered array value %d : 0x%x", i,
				(uint)value);
		}
	}
	if (i == 0) {
		...
		const char *buf, size_t count)
{
	...
	if (i == 0) {
		...
		if (!sii_drv_set_custom_msg(drv_context,
			bus_id, dfp_contract, true))
			status = -ENODATA;
		else
			status = count;
	}
...
static int rpt_device_dbg_i2c_reg_xfer(
	struct sii70xx_drv_context *drv_context, u16 offset, u16 count,
 ...
 *
 */
ssize_t show_reg_access_page(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct sii70xx_drv_context *drv_context = dev_get_drvdata(dev);
	int status = -EINVAL;
...
struct device_attribute driver_attribs[] = {
	__ATTR(SYS_ATTR_NAME_SRC_CAP, 0220,
		NULL, set_src_cap),
	__ATTR(SYS_ATTR_NAME_VDM_DISC_IDENTITY, 0220,
		NULL, set_vdm_disc_identity),
	__ATTR(SYS_ATTR_NAME_VDM_EXIT_MODE, 0220,
		NULL, set_vdm_exit_mode),
	__ATTR(SYS_ATTR_NAME_SET_PR_SWAP, 0220,
		NULL, set_pr_swap),
	__ATTR(SYS_ATTR_NAME_SET_DR_SWAP, 0220,
		NULL, set_dr_swap),
	__ATTR(SYS_ATTR_NAME_SET_VCONN_SWAP, 0220,
		NULL, set_vconn_swap),
	__ATTR(SYS_ATTR_NAME_SET_CUSTOM_MESSAGE, 0220,
		NULL, set_custom_msg),
	__ATTR_NULL
};


static DEVICE_ATTR(src_cap, 0220, NULL, set_src_cap);
static DEVICE_ATTR(vdm_disc_identity, 0220, NULL, set_vdm_disc_identity);
static DEVICE_ATTR(vdm_exit_mode, 0220, NULL, set_vdm_exit_mode);
static DEVICE_ATTR(pr_swap, 0220, NULL, set_pr_swap);
static DEVICE_ATTR(dr_swap, 0220, NULL, set_dr_swap);
static DEVICE_ATTR(vconn_swap, 0220, NULL, set_vconn_swap);
static DEVICE_ATTR(custom_msg, 0220, NULL, set_custom_msg);

static struct attribute *sii7033_attribs[] = {
	&dev_attr_src_cap.attr,
	&dev_attr_vdm_disc_identity.attr,
	&dev_attr_vdm_exit_mode.attr,
	&dev_attr_pr_swap.attr,
	&dev_attr_dr_swap.attr,
	&dev_attr_vconn_swap.attr,
	&dev_attr_custom_msg.attr,
	NULL
};
*/
struct device_attribute src_cap_attr =
__ATTR(SYS_ATTR_NAME_SRC_CAP, 0220,
	NULL, set_src_cap);

struct device_attribute vdm_disc_identity_attr =
__ATTR(SYS_ATTR_NAME_VDM_DISC_IDENTITY, 0220,
	NULL, set_vdm_disc_identity);

struct device_attribute vdm_exit_mode_attr =
__ATTR(SYS_ATTR_NAME_VDM_EXIT_MODE, 0220,
	NULL, set_vdm_exit_mode);

struct device_attribute pr_swap_attr =
__ATTR(SYS_ATTR_NAME_SET_PR_SWAP, 0220,
	NULL, set_pr_swap);

struct device_attribute dr_swap_attr =
__ATTR(SYS_ATTR_NAME_SET_DR_SWAP, 0220,
	NULL, set_dr_swap);

struct device_attribute vconn_swap_attr =
__ATTR(SYS_ATTR_NAME_SET_VCONN_SWAP, 0220,
	NULL, set_vconn_swap);

struct device_attribute custom_msg_attr =
__ATTR(SYS_ATTR_NAME_SET_CUSTOM_MESSAGE, 0220,
	NULL, set_custom_msg);

struct attribute *sii7033_attribs[] = {
	&src_cap_attr.attr,
	&vdm_disc_identity_attr.attr,
	&vdm_exit_mode_attr.attr,
	&pr_swap_attr.attr,
	&dr_swap_attr.attr,
	&vconn_swap_attr.attr,
	&custom_msg_attr.attr,
	NULL
};

static const struct attribute_group sii7033_attr_group = {
	.attrs = sii7033_attribs,
};

#if defined(I2C_DBG_SYSFS)
struct device_attribute reg_access_page_attr =
__ATTR(SYS_ATTR_NAME_REG_ACCESS_PAGE, 0660,
	show_reg_access_page, set_reg_access_page);

struct device_attribute reg_access_offset_attr =
__ATTR(SYS_ATTR_NAME_REG_ACCESS_OFFSET, 0660,
	show_reg_access_offset, set_reg_access_offset);

struct device_attribute reg_access_length_attr =
__ATTR(SYS_ATTR_NAME_REG_ACCESS_LENGTH, 0660,
	show_reg_access_length, set_reg_access_length);

struct device_attribute reg_access_data_attr =
__ATTR(SYS_ATTR_NAME_REG_ACCESS_DATA, 0660,
...
struct attribute_group reg_access_attribute_group = {
	.name = __stringify(SYS_OBJECT_NAME_REG_ACCESS), .attrs =
			reg_access_attrs };
#endif
...
bool sii_drv_sysfs_init(struct sii70xx_drv_context *pdev, struct device *dev)
{
	...
	if (pdev->usbpd_class == NULL) {
		...
		//pdev->usbpd_class->dev_groups = &sii7033_attr_group;

		ret = alloc_chrdev_region(&pdev->dev_num, 0, 1, "sii70xx");
		if (ret) {
			pr_warning("%s: alloc_char_region failed\n" , __func__);
			return false;
		}
	...
	pdev->dev = device_create(pdev->usbpd_class, dev,
		pdev->dev_num, pdev, "%s", "sii70xx");

	if (IS_ERR(pdev->dev)) {
		ret = PTR_ERR(pdev->dev);
		pr_warning("%s: device_create failed %s %d\n" , __func__,
			"sii70xx", ret);
		return false;
	}

	ret = sysfs_create_group(&pdev->dev->kobj, &sii7033_attr_group);
	if (ret) {
	...
	if (ret)
		pr_warning("Debug sysfs_create_group failed:%d\n", ret);
...
void usbpd_event_notify(struct sii70xx_drv_context *pdev, unsigned int events,
	...
	case PD_UFP_ATTACHED:
		sysfs_notify(&pdev->dev->kobj, NULL,
			__stringify(SYS_ATTR_NAME_UFP_CONN));

		strlcpy(event_string, "UFP=attached",
	...
	case PD_UFP_DETACHED:
		...
		strlcpy(event_string, "UFP=detached",
			USBPD_EVENT_STRING_LEN);
		kobject_uevent_env(&pdev->dev->kobj, KOBJ_CHANGE,
			envp);
		break;
	case PD_DFP_ATTACHED:
		sysfs_notify(&pdev->dev->kobj, NULL,
			__stringify(SYS_ATTR_NAME_DFP_CONN));

		strlcpy(event_string, "DFP=attached",
			USBPD_EVENT_STRING_LEN);
		kobject_uevent_env(&pdev->dev->kobj, KOBJ_CHANGE,
			envp);
		break;
	case PD_DFP_DETACHED:
		...
		strlcpy(event_string, "DFP=detached",
			USBPD_EVENT_STRING_LEN);
		kobject_uevent_env(&pdev->dev->kobj, KOBJ_CHANGE,
			envp);
		break;
	case PD_POWER_LEVELS:
		sysfs_notify(&pdev->dev->kobj, NULL,
			__stringify(SYS_ATTR_NAME_POWER_LEVELS));

		strlcpy(event_string, "5V, 900mv",
			USBPD_EVENT_STRING_LEN);
		kobject_uevent_env(&pdev->dev->kobj, KOBJ_CHANGE,
			envp);
		break;
	case PD_UNSTRUCTURED_VDM:
		...
		strlcpy(event_string,
			"Unstructred VDM set for Below Voltage levels",
			USBPD_EVENT_STRING_LEN);
		kobject_uevent_env(&pdev->dev->kobj, KOBJ_CHANGE,
			envp);
		break;
