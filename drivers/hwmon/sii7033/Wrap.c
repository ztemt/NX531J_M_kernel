	...
	for (i = 0; i < I2C_RETRY_MAX; i++) {
		...
		if (ret != 2) {
			pr_err("%s:%d I2c read failed, retry 0x%02x:0x%02x\n"
				, __func__, __LINE__, device_id, offset);
			msleep(20);
		} else {
			break;
		}
...
int sii_create_timer(struct usbpd_device *context, void (*callback_handler)
	(void *callback_param), void *callback_param,
		struct usbpd_timer *usbpdtmr, uint32_t time_msec,
			bool periodicity)
{
	...
	if (new_timer == NULL) {
		pr_debug("Invalid Timer Instance\n");
		return -ENOMEM;
	}
...
void sii_destroy_timer_support(struct usbpd_device *drv_context)
{
	...
	while (!list_empty(&drv_context->timer_list)) {
		sii_timer = list_first_entry(&drv_context->timer_list,
...
int is_timer_handle_valid(struct usbpd_device *drv_context,
	void *timer_handle)
{
	...
	do {
		if (sii_timer->timer_cnt == 0)
			ret = wait_event_interruptible(
					sii_timer->timer_run_wait_queue,
					sii_timer->wake_flag);
			...
			bool periodicity)
{
	...
	new_timer->timer_task_thread = kthread_create(s_timer_thread,
			(void *)new_timer, "kthread");
...
int sii_timer_delete(void **timer_handle)
{
	...
	wake_up_interruptible(&sii_timer->timer_run_wait_queue);
	wait_event_interruptible(sii_timer->timer_del_wait_queue,
			sii_timer->del_flag);
