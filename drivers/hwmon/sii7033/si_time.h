...
#define SI_TIME_H
#include "Wrap.h"
int sii_start_timer(void *context, void **timer_handle);//	uint32_t time_msec, bool peridicity);
...
struct timer_obj {
	...
	struct task_struct *timer_task_thread;
...
int sii_stop_timer(struct usbpd_device *context,
	struct usbpd_timer *usbpdtmr);
int sii_start_timer(struct usbpd_device *context,
	struct usbpd_timer *usbpdtmr);
int sii_delete_timer(struct usbpd_device *context,
	struct usbpd_timer *usbpdtmr);
#else
