...
static bool sii_pd_vdm_handler(struct sii_usbpd_protocol *pd,
struct pd_inq *req);
...
void sii_rcv_usbpd_data(struct sii70xx_drv_context *drv_context)
{
	...
	sii_platform_block_read8(REG_ADDR__PDRXBUF0,
			pUsbpd_prtlyr->rx_msg->recv_msg,
			count);
	...
	usbpd_process_msg(pUsbpd_prtlyr, pUsbpd_prtlyr->
		rx_msg->recv_msg, count);
}
	...
	uint16_t pd_header, uint32_t *buf, uint8_t length)
{
	...
	if (!buf || length == 0) {
		...
		memcpy((pd->cb_params->send_data + sizeof(pd_header)),
				(uint8_t *)buf, length);
	}
...
bool usbpd_xmit_ctrl_msg(struct sii_usbpd_protocol *pd, enum ctrl_msg type)
{
	...
	pd_header = SII_USBPD_HEADER(type, pd->pwr_role, pd->data_role,
			0, 0);
...
bool usbipd_send_soft_reset(struct sii70xx_drv_context *drv_context,
	enum ctrl_msg type)
{
	struct sii_usbpd_protocol *pUsbpd_prtlyr =
		(struct sii_usbpd_protocol *)
...
bool sii_usbpd_xmit_data_msg(struct sii_usbpd_protocol *pd,
	enum data_msg type,
	uint32_t *src_pdo, uint8_t count)
{
	uint16_t pd_header;
	bool b_result;
	...
	pd_header = SII_USBPD_HEADER(type, pd->pwr_role,
		pd->data_role,
		0, count);

	b_result = sii_submit_pdmsg(pd, pd_header,
		src_pdo, count);
...
bool sii_process_data_msg(struct sii_usbpd_protocol *pd,
struct pd_inq *req)
{
	if (!req) {
		pr_debug("%s: Enter\n", __func__);
	...
	memset(pd->cb_params, 0, sizeof(struct
		pd_cb_params));
	...
	case SRCCAP:
		...
		set_bit(SRC_CAP_RCVD, &pd->cb_params->
			sm_cmd_inputs);
		break;
	case REQ:
		pr_info(": REQ RCVD. :\n");
		set_bit(REQ_RCVD, &pd->cb_params->
			sm_cmd_inputs);
		break;
	case BIST:
		pr_info(": BIST RCVD. :\n");
		set_bit(BIST_RCVD, &pd->cb_params->
			sm_cmd_inputs);
		parse_bist(pd, req);
		return true;
		break;
	case SNKCAP:
		...
		set_bit(SNK_CAP_RCVD, &pd->cb_params->
			sm_cmd_inputs);
		break;
	case VDM:
...
bool sii_process_ctrl_cmds(struct sii_usbpd_protocol *pd,
struct pd_inq *req)
{
	if (!req) {
		pr_debug("%s: Error\n", __func__);
	...
	case CTRL_MSG__GET_SRC_CAP:
		...
		set_bit(GET_SOURCE_CAP_RCVD,
				&pd->cb_params->sm_cmd_inputs);
		break;
	case CTRL_MSG__GET_SINK_CAP:
		pr_info(": GET_SINK_CAP :\n");
		set_bit(GET_SINK_CAP_RCVD,
				&pd->cb_params->sm_cmd_inputs);
		break;
	case CTRL_MSG__DR_SWAP:
		pr_info(": DR_SWAP :\n");
		set_bit(DR_SWAP_RCVD,
				&pd->cb_params->sm_cmd_inputs);
		break;
...
static void parse_vdm_identity(struct sii_usbpd_protocol *pd,
	struct pd_inq *req)
{
	bool is_usb_host, is_usb_device, is_modal_op_supp;
	...
	case IDH_PTYPE_UNDEF:
		...
		/*if (svid0 != 0xff00) {
			result =
			usbpd_svdm_init_resp_nak(pd,
			CMD_DISCOVER_IDENT, svid0,
			false, NULL);
			clear_bit(SVDM_DISC_IDEN_RCVD, &pd->cb_params->
				svdm_sm_inputs);
		}*/
		/*else
			result = usbpd_svdm_init_resp(pd,
			CMD_DISCOVER_IDENT,
			false, NULL);*/
		/*if (result)
	...
	case IDH_PTYPE_HUB:
		...
		pr_info("usb product id => %X\n ", ((req->
			msgdata[VDO_I(PRODUCT)] >> 16) & 0xFFFF));
		pr_info("usb bcd device id => %X\n ", ((req->
			msgdata[VDO_I(PRODUCT)]) & 0xFFFF));
		break;
	...
	case IDH_PTYPE_PCABLE:
		...
		pr_info("CABLE TYPE => %s\n",
			((req->msgdata[VDO_I(CABLE)] >> 17) & 0x1) ?
			"receptacle" : "plug");
	...
	case IDH_PTYPE_AMA:
		...
		pr_info("usb product id => %X\n ",
			((req->msgdata[VDO_I(PRODUCT)] >> 16) &
					0xFFFF));
		pr_info("usb bcd device id => %X\n ",
			((req->msgdata[VDO_I(PRODUCT)]) & 0xFFFF));
		pd->pd_hw_ver     = PD_VDO_AMA_HW_VER(req->
			msgdata[VDO_I(AMA)]);
		pr_info("HW Version => %X\n", pd->pd_hw_ver);
		pd->pd_fw_ver  = PD_VDO_AMA_FW_VER(req->
			msgdata[VDO_I(AMA)]);
		pr_info("FW Version => %X\n", pd->pd_fw_ver);
		pd->ss_tx1_func   = PD_VDO_AMA_SS_TX1(req->
			msgdata[VDO_I(AMA)]);
		pr_info("pd->ss_tx1_func => %X\n", pd->ss_tx1_func);
		pd->ss_tx2_func  = PD_VDO_AMA_SS_TX2(req->
			msgdata[VDO_I(AMA)]);
		pr_info("pd->ss_tx2_func => %X\n", pd->ss_tx2_func);
		pd->ss_rx1_func   = PD_VDO_AMA_SS_RX1(req->
			msgdata[VDO_I(AMA)]);
		pr_info("pd->ss_rx1_func => %X\n", pd->ss_rx1_func);
		pd->ss_rx2_func   = PD_VDO_AMA_SS_RX2(req->
			msgdata[VDO_I(AMA)]);
		pr_info("pd->ss_rx2_func => %X\n", pd->ss_rx2_func);
		pd->vconn	  = PD_VDO_AMA_VCONN_PWR(req->
			msgdata[VDO_I(AMA)]);
		pr_info("pd->vconn => %X\n", pd->vconn);
		pd->vconn_req     = PD_VDO_AMA_VCONN_REQ(req->
			msgdata[VDO_I(AMA)]);
		pr_info("pd->vconn_req => %X\n", pd->vconn_req);
		pd->vbus_req      = PD_VDO_AMA_VBUS_REQ(req->
			msgdata[VDO_I(AMA)]);
		pr_info("pd->vbus_req => %X\n", pd->vbus_req);
		pd->usb_ss_supp	  = PD_VDO_AMA_SS_SUPP(req->
			msgdata[VDO_I(AMA)]);
...
static void parse_svid(struct sii_usbpd_protocol *pd, struct pd_inq *req)
{
	...
	for (i = 0; i < (MSG_CNT(req->command) - 1); i++) {
		...
		if (svid0 == 0xFF01) {
			...
			set_bit(DISPLAY_PORT_RCVD, &pd->
			cb_params->svdm_sm_inputs);
			break;
		}
		pr_info("svid0 => %X\n", svid0);
...
static void parse_modes(struct sii_usbpd_protocol *pd,
struct pd_inq *req)
{
	int i, j = 0;
	uint32_t *ptr = ((req->msgdata) + 0);
	...
	for (i = 0, j = 0; i < (MSG_CNT(req->command) - 1) &&
		j < SVID_DISCOVERY_MAX; i++, j++) {
		if (i == SVID_DISCOVERY_MAX) {
			pr_debug("ERR:SVIDCNT\n");
			break;
		}
...
static void init_resp_ack(struct sii_usbpd_protocol *pd,
struct pd_inq *req)
{
	...
	case DISCOVER_IDENTITY:
		...
		set_bit(SVDM_DISC_IDEN_ACK_RCVD, &pd->
				cb_params->svdm_sm_inputs);
		/*parse_vdm_identity(pd, req);*/
		break;
	case DISCOVER_SVID:
		pr_info("SVDM_CMD_DISCOVER_SVID ack\n");
		set_bit(DISCOVER_SVID_ACK_RCVD, &pd->
				cb_params->svdm_sm_inputs);
		parse_svid(pd, req);
		break;
	case DISCOVER_MODE:
		pr_info("SVDM_CMD_DISCOVER_MODE ack\n");
		set_bit(DISCOVER_MODE_ACK_RCVD, &pd->
				cb_params->svdm_sm_inputs);
		parse_modes(pd, req);
		break;
	case ENTER_MODE:
		pr_info("SVDM_CMD_ENTER_MODE ack\n");
		set_bit(ENTER_MODE_ACK_RCVD, &pd->cb_params->
			svdm_sm_inputs);
		break;
	case EXIT_MODE:
		pr_info("SVDM_CMD_EXIT_MODE ack\n");
		set_bit(EXIT_MODE_ACK_RCVD, &pd->cb_params->
			svdm_sm_inputs);
		break;
	case ATTENTION:
		pr_info("SVDM_CMD_ATTENTION ack\n");
		set_bit(VDM_ATTENTION_ACK_RCVD, &pd->
				cb_params->svdm_sm_inputs);
		break;
	default:
...
static void init_resp_nack(struct sii_usbpd_protocol *pd, struct pd_inq *req)
{
	...
	case DISCOVER_IDENTITY:
		...
		set_bit(SVDM_DISC_IDEN_NACK_RCVD, &pd->
				cb_params->svdm_sm_inputs);
		break;
	case DISCOVER_SVID:
		pr_debug("SVDM_CMD_DISCOVER_SVID nack\n");
		set_bit(DISCOVER_SVID_NACK_RCVD, &pd->
				cb_params->svdm_sm_inputs);
		break;
	case DISCOVER_MODE:
		pr_debug("SVDM_CMD_DISCOVER_MODE nack\n");
		set_bit(DISCOVER_MODE_NACK_RCVD, &pd->
				cb_params->svdm_sm_inputs);
		break;
	case ENTER_MODE:
		pr_debug("SVDM_CMD_ENTER_MODE nack\n");
		set_bit(ENTER_MODE_NACK_RCVD, &pd->cb_params->
			svdm_sm_inputs);
		break;
	case EXIT_MODE:
		pr_debug("SVDM_CMD_EXIT_MODE nack\n");
		set_bit(EXIT_MODE_NACK_RCVD, &pd->cb_params->
			svdm_sm_inputs);
		break;
	case ATTENTION:
		pr_debug("SVDM_CMD_ATTENTION nack\n");
		set_bit(VDM_ATTENTION_NACK_RCVD, &pd->
			cb_params->svdm_sm_inputs);
		break;
	default:
...
static void init_busy(struct sii_usbpd_protocol *pd, struct pd_inq *req)
{
	...
	case DISCOVER_IDENTITY:
		...
		set_bit(SVDM_DISC_IDEN_BUSY_RCVD, &pd->
			cb_params->svdm_sm_inputs);
		break;
	case DISCOVER_SVID:
		pr_debug("SVDM_CMD_DISCOVER_SVID busy\n");
		set_bit(DISCOVER_SVID_BUSY_RCVD, &pd->
			cb_params->svdm_sm_inputs);
		break;
	case DISCOVER_MODE:
		pr_debug("SVDM_CMD_DISCOVER_MODE busy\n");
		set_bit(DISCOVER_MODE_BUSY_RCVD, &pd->
			cb_params->svdm_sm_inputs);
		break;
	case ENTER_MODE:
		pr_debug("SVDM_CMD_ENTER_MODE busy\n");
		set_bit(ENTER_MODE_BUSY_RCVD, &pd->cb_params->
			svdm_sm_inputs);
		break;
	case EXIT_MODE:
		pr_debug("SVDM_CMD_EXIT_MODE busy\n");
		set_bit(EXIT_MODE_BUSY_RCVD, &pd->cb_params->
			svdm_sm_inputs);
		break;
	case ATTENTION:
		pr_debug("SVDM_CMD_ATTENTION busy\n");
		set_bit(VDM_ATTENTION_BUSY_RCVD, &pd->
			cb_params->svdm_sm_inputs);
		break;
	default:
...
static void parse_enter_mode(struct sii_usbpd_protocol *pd,
	struct pd_inq *req)
{
	uint16_t svids;
	uint8_t opos;
	...
	if (svids != 0xFF02)
		result = usbpd_svdm_init_resp_nak(pd, CMD_ENTER_MODE, svids,
			false, NULL);
	if (result)
		pr_debug("undefined done\n");
	else
...
static void parse_exit_mode(struct sii_usbpd_protocol *pd,
	struct pd_inq *req)
{
	uint16_t svids;
	uint8_t opos;
	...
	if (svids == 0xFF02) {
		...
		set_bit(MHL_EXITED, &pd->cb_params->
			svdm_sm_inputs);
	} else {
		pr_debug("Undefined mode exited\n");
		set_bit(MHL_NOT_EXITED, &pd->cb_params->
			svdm_sm_inputs);
	}
	...
	if (svids != 0xFF02)
		result = usbpd_svdm_init_resp_nak(pd, CMD_EXIT_MODE, svids,
			false, NULL);
	if (result)
		pr_debug("undefined done\n");
	else
...
bool send_custom_vdm_msg_resp(struct sii_usbpd_protocol *pd,
	enum data_msg msg_type)
{
	bool b_result;
	...
	pd_header = SII_USBPD_HEADER(VDM, pd->pwr_role,
			pd->data_role, 0, 1);
...
bool send_custom_vdm_message(struct sii_usbpd_protocol *pd,
	enum data_msg msg_type, uint8_t type)
{
	bool b_result;
	...
	pd_header = SII_USBPD_HEADER(VDM, 0,
			1, 0, 2);
...
unsigned int svdm_get_identity_info(struct sii_usbpd_protocol *pd,
		uint32_t *vdm_data, uint8_t cmd)
{
	return USBPD_OBJ_2;
}
	...
	enum data_msg msg_type, uint8_t type, uint8_t pdo)
{
	...
	switch (type) {
	case DISCOVER_IDENTITY:
		vdm_header = SVDM_HEADER(0xFF00, 1, 0, 0,
			INITIATOR,
			DISCOVER_IDENTITY);
		pr_info("DISCOVER_IDENTITY\n ");
		break;
	...
	case DISCOVER_SVID:
		if (pdo == 0x01) {
			...
			vdm_header = SVDM_HEADER(0xFF01, 1, 0, 0,
				INITIATOR,
				DISCOVER_SVID);
		}
		pr_info("DISCOVER_SVID\n ");
	...
	case DISCOVER_MODE:
		if (pdo == 0x01) {
			vdm_header = SVDM_HEADER(0xFF02, 1, 0, 0,
				INITIATOR,
				DISCOVER_MODE);
		} else if (pdo == 0x02) {
			vdm_header = SVDM_HEADER(0xFF01, 1, 0, 0,
	...
	case ENTER_MODE:
		if (pdo == 0x01) {
			vdm_header = SVDM_HEADER(0xFF02, 1, 0, 1,
				INITIATOR,
				ENTER_MODE);
		} else {
			vdm_header = SVDM_HEADER(0xFF01, 1, 0, 1,
				INITIATOR,
				ENTER_MODE);
		}
		pr_info("ENTER_MODE\n ");
...
bool usbpd_svdm_init_resp(struct sii_usbpd_protocol *pd,
	uint8_t cmd, bool is_rcvd, uint32_t *vdo)
{
	...
	if (!vdo) {
		switch (cmd) {
		case DISCOVER_IDENTITY:
			vdm_data[VDM_HDR]   = SVDM_HEADER(0xFF00, 1, 0, 0,
				RESP_ACK, cmd);
			vdm_data[VDM_IDH]   = SVDM_IDH(1, 1, 5, 0, 0x8976);
			vdm_data[VDM_CERT]  = SII_PREPARE_CERT_STAT_HEADER(
				0, 0xFFFFF);
			vdm_data[VDM_PTYPE] = SII_PREPARE_PRODUCT_VDO_HEADER(
				0xFFFF, 0xFFFF);
		...
		case DISCOVER_SVID:
			...
			break;
		case DISCOVER_MODE:
			vdm_data[VDM_HDR] = SVDM_HEADER(0xFF02, 1, 0, 0,
					RESP_ACK, cmd);
	...
	pd_header = SII_USBPD_HEADER(VDM,
			pd->pwr_role,
			pd->data_role,
			0,
			no_of_objs);
	...
	return sii_submit_pdmsg(pd, pd_header,
			vdm_data, no_of_objs);
}
	...
	uint8_t cmd, uint16_t svid0, bool is_rcvd, uint32_t *vdo)
{
	...
	switch (cmd) {
	case DISCOVER_IDENTITY:
		vdm_data[VDM_HDR]   = SVDM_HEADER(svid0, 1, 0, 0,
			RESP_NACK, cmd);
		/*vdm_data[VDM_IDH]   = 0;*/
		/*vdm_data[VDM_CERT]  = 0;*/
	...
	case DISCOVER_MODE:
		...
		break;
	case ENTER_MODE:
		vdm_data[VDM_HDR] = SVDM_HEADER(svid0, 1, 0, 1,
			RESP_NACK, cmd);
		no_of_objs  = USBPD_OBJ_1;
		break;
	case EXIT_MODE:
...
static void sii_parse_pd_msg(unsigned char *rx_buf, char count,
		struct pd_inq *req)
{
	req->command = (rx_buf[1] << 8) | (rx_buf[0] << 0);
	memcpy(req->msgdata, (uint32_t *)(rx_buf+
		sizeof(req->command)),
			count);
	req->count   = count;
}

bool usbpd_process_msg(struct sii_usbpd_protocol *pd, uint8_t *buf,
	uint8_t count)
{
	bool b_result = false;
	...
	if (count == 2) {
		b_result = sii_process_ctrl_cmds(pd,
			pd->in_command);
		goto exit;
	} else {
		b_result = sii_process_data_msg(pd,
			pd->in_command);
		if (b_result)
			clean_inq_req(pd);
		goto exit;
...
void update_data_role(void *context,
		uint8_t updated_role)
{
...
void update_pwr_role(void *context,
		uint8_t updated_role)
{
	struct sii70xx_drv_context *drv_context =
		...
		bool (*event_notify_fn)(void*, struct pd_cb_params *))
{
	...
	prot_lyr->cb_params = kzalloc(
		sizeof(struct pd_cb_params), GFP_KERNEL);
	...
	prot_lyr->in_command->msgdata = kzalloc(MAX_PD_PAYLOAD,
			GFP_KERNEL);
	SII_PLATFORM_DEBUG_ASSERT(prot_lyr->in_command->msgdata);
	if (!prot_lyr->in_command->msgdata) {
		pr_debug("%s: Error...msgdata\n", __func__);
		kfree(prot_lyr);
		kfree(prot_lyr->cb_params);
		kfree(prot_lyr->in_command);
		return NULL;
	}
	prot_lyr->rx_msg = kzalloc(sizeof(struct si_data_intf),
			GFP_KERNEL);

	if (!prot_lyr->rx_msg) {
		pr_err("%s: Error...msgdata\n", __func__);
		kfree(prot_lyr);
		kfree(prot_lyr->cb_params);
		kfree(prot_lyr->in_command);
