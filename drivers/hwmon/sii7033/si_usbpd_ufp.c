...
/*********************************************************************/
int sii_usbpd_snk_alt_mode_req(struct sii_usbp_policy_engine *pUsbpd,
	uint8_t portnum)
{
	...
	if (pUsbpd->state == PE_SNK_Ready) {
		...
		queue_work(pUsbpd->ufp_work_queue, &pUsbpd->pd_ufp_work);
		pr_info("\n Alt mode initiated\n");
		return true;
	} else {
...
int sii_usbpd_snk_pwr_swap(struct sii_usbp_policy_engine *pUsbpd,
	uint8_t portnum)
{
	if (!pUsbpd->pd_connected) {
		pr_info("\n !!!!!PD is not Established!!!!\n");
...
int sii_usbpd_snk_data_swap(struct sii_usbp_policy_engine *pUsbpd,
	uint8_t portnum)
{
	if (!pUsbpd->pd_connected) {
		pr_info("\n !!!!!PD is not Established!!!!\n");
...
int sii_usbpd_give_src_cap(struct sii_usbp_policy_engine *pUsbpd,
	uint8_t portnum)
{
	if (!pUsbpd->pd_connected) {
		pr_info("\n !!!!!PD is not Established!!!!\n");
...
bool usbpd_sink_timer_create(struct sii_usbp_policy_engine *pUsbpd)
{
	...
	if (!pUsbpd->usbpd_inst_sink_wait_cap_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register sink_wait_cap_tmr!\n");
			goto exit;
		}
	}
	if (!pUsbpd->usbpd_inst_snk_sendr_resp_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register SenderResponseTimer timer!\n");
			goto exit;
		}
	}
	if (!pUsbpd->usbpd_inst_ps_trns_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register PSTransitionTimer timer!\n");
			goto exit;
		}
	}
	if (!pUsbpd->usbpd_ps_source_off_tmr) {
		...
		if (status != 0) {
			pr_warning("Failed to register PSSourceOffTimer!\n");
			goto exit;
		}
	}
	return true;
...
bool usbpd_sink_timer_delete(struct sii_usbp_policy_engine *pUsbpd)
{
	...
	if (pUsbpd->usbpd_ps_source_off_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete inst_ps_src_off_tmr!\n");
			return false;
		}
	}
	if (pUsbpd->usbpd_inst_ps_trns_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete inst_ps_trns_tmr!\n");
			return false;
		}
	}
	...
	if (pUsbpd->usbpd_inst_sink_wait_cap_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete inst_sink_wait_cap_tmr!\n");
			return false;
		}
	}
	...
	if (pUsbpd->usbpd_inst_sink_act_tmr) {
		...
		if (rc != 0) {
			pr_warning("Failed to delete inst_sink_act_tmr!\n");
			return false;
		}
	}
	return true;
...
bool sii_snk_pr_swap_engine(struct sii_usbp_policy_engine *pdev)
{
	...
	case PE_PRS_SNK_SRC_Accept_Swap:
		...
		pr_info("  SEND_COMMAND: ACCEPT\n");
		pr_info("\n========================\n");
		usbpd_xmit_ctrl_msg(pUsbpd_prtlyr,
			CTRL_MSG__ACCEPT);
		pdev->pr_swap_state =
...
bool sii_snk_vdm_mode_engine(struct sii_usbp_policy_engine *pdev)
{
	...
	clear_bit(VDM_MSG_RCVD, &pdev->
			intf.param.svdm_sm_inputs);

	if (test_bit(SVDM_DISC_IDEN_RCVD, &pdev->
		intf.param.svdm_sm_inputs)) {
		clear_bit(SVDM_DISC_IDEN_RCVD, &pdev->
			intf.param.svdm_sm_inputs);
		pdev->alt_mode_state = PE_UFP_VDM_Get_Identity;
	}
	if (test_bit(DISCOVER_SVID_RCVD, &pdev->
		intf.param.svdm_sm_inputs)) {
		clear_bit(DISCOVER_SVID_RCVD, &pdev->
			intf.param.svdm_sm_inputs);
		pdev->alt_mode_state = PE_UFP_VDM_Get_SVIDs;
	}
	if (test_bit(DISCOVER_MODE_RCVD, &pdev->
		intf.param.svdm_sm_inputs)) {
		clear_bit(DISCOVER_MODE_RCVD, &pdev->
			intf.param.svdm_sm_inputs);
		pdev->alt_mode_state = PE_UFP_VDM_Get_Modes;
	}
	if (test_bit(ENTER_MODE_RCVD, &pdev->
		intf.param.svdm_sm_inputs)) {
		clear_bit(ENTER_MODE_RCVD, &pdev->
			intf.param.svdm_sm_inputs);
		pdev->alt_mode_state = PE_UFP_VDM_Evaluate_Mode_Entry;
	}
	if (test_bit(EXIT_MODE_RCVD, &pdev->
		intf.param.svdm_sm_inputs)) {
		clear_bit(EXIT_MODE_RCVD, &pdev->
			intf.param.svdm_sm_inputs);
	...
	case PE_UFP_VDM_Send_Identity:
		...
		usbpd_svdm_init_resp(pUsbpd_prtlyr,
				CMD_DISCOVER_IDENT,
				false, NULL);
		pdev->alt_mode_state =
...
void sink_policy_engine(struct work_struct *w)
{
	struct sii_usbp_policy_engine *pdev =
	...
	if (!down_interruptible(&drv_context->isr_lock)) {
		pr_debug("SINK STATE - %d %d %lx\t",
			pdev->state, pdev->pr_swap_state, pdev->intf.
					param.sm_cmd_inputs);
		switch (pdev->state) {
		case PE_SNK_Startup:
		...
		case PE_SNK_Wait_for_Capabilities:
			sii_timer_start(&(pdev->
				usbpd_inst_sink_wait_cap_tmr));
			if (test_bit(SRC_CAP_RCVD, &pdev->
				intf.param.sm_cmd_inputs)) {
				clear_bit(SRC_CAP_RCVD, &pdev->
					intf.param.sm_cmd_inputs);
				sii_timer_stop(&pdev->
					usbpd_inst_sink_wait_cap_tmr);
				pdev->next_state =
		...
		case PE_SNK_Select_Capability:
			...
			sii_usbpd_xmit_data_msg(pUsbpd_protlyr, REQ,
				&pdev->rdo,
					1);
			pdev->next_state =
		...
		case PE_SNK_Wait_Request_Good_Crc_Received:
			if (pdev->tx_good_crc_received) {
				sii_timer_start(&(pdev->
					usbpd_inst_snk_sendr_resp_tmr));
				if (test_bit(ACCEPT_RCVD,
		...
		case PE_SNK_Transition_Sink:
			if (test_bit(ACCEPT_RCVD, &pdev->intf.
					param.sm_cmd_inputs)) {
				sii_timer_stop(&
					(pdev->
					usbpd_inst_snk_sendr_resp_tmr));
				...
				break;
					...
					sm_cmd_inputs)) {
				clear_bit(REJECT_RCVD, &pdev->intf.param.
					sm_cmd_inputs);
				sii_timer_stop(&
					(pdev->usbpd_inst_snk_sendr_resp_tmr));
				pdev->next_state =  PE_SNK_Ready;
				work = 1;
			} else if (test_bit(WAIT_RCVD, &pdev->intf.
					param.sm_cmd_inputs)) {
				sii_timer_stop(&(pdev->
					usbpd_inst_snk_sendr_resp_tmr));
				pdev->next_state =  PE_SNK_Ready;
		...
		case PE_SNK_Wait_Ps_Rdy_Received:
			if (test_bit(PS_RDY_RCVD, &pdev->intf.
				param.sm_cmd_inputs)) {
				clear_bit(PS_RDY_RCVD, &pdev->intf.
					param.sm_cmd_inputs);
				sii_timer_stop(&
					(pdev->usbpd_inst_ps_trns_tmr));
				pdev->next_state = PE_SNK_Ready;
		...
		case PE_SNK_Ready:
			...
			if (pdev->dr_swap.done) {
				if (pdev->custom_msg) {
					...
					pdev->next_state = PE_SNK_Ready;
					work = 1;
					queue_work(pdev->ufp_work_queue,
						&pdev->pd_ufp_work);
					return;
				} else {
					pdev->dr_swap.done = false;
			...
			if (test_bit(SOFT_RESET_RCVD,
					&pdev->intf.param.
					sm_cmd_inputs)) {
				clear_bit(SOFT_RESET_RCVD, &pdev->
					intf.param.sm_cmd_inputs);
				pr_info("\n========================\n");
				pr_info("  SEND_COMMAND: ACCEPT\n");
				pr_info("\n========================\n");
			...
			/****************************************************/
			if (test_bit(VCONN_SWAP_RCVD, &pdev->
				intf.param.sm_cmd_inputs)) {
				clear_bit(VCONN_SWAP_RCVD, &pdev->
					intf.param.sm_cmd_inputs);
			...
			if (test_bit(DR_SWAP_RCVD, &pdev->intf.param.
				sm_cmd_inputs)) {
			...
			/*****************************************************/
			if (test_bit(GOTOMIN_RCVD, &pdev->intf.
				param.sm_cmd_inputs)) {
				clear_bit(GOTOMIN_RCVD, &pdev->
					intf.param.sm_cmd_inputs);
				work = 1;
				pdev->next_state = PE_SNK_Transition_Sink;
				pdev->alt_mode_cmnd_xmit = false;
			...
			if (test_bit(WAIT_RCVD, &pdev->intf.
				param.sm_cmd_inputs)) {
				sii_timer_start(&(pdev->
					usbpd_inst_sink_req_tmr));
				clear_bit(WAIT_RCVD, &pdev->
					intf.param.sm_cmd_inputs);
				work = 1;
				pdev->next_state = PE_SNK_Ready;
				pdev->alt_mode_cmnd_xmit = false;
			...
			if (test_bit(GET_SINK_SRC_CAP_RCVD, &pdev->
				intf.param.sm_cmd_inputs)) {
				clear_bit(GET_SINK_SRC_CAP_RCVD, &pdev->
					intf.param.sm_cmd_inputs);
				work = 1;
		...
		case PE_SNK_Give_Sink_Cap:
			...
			sii_usbpd_get_snk_cap(pdev, pUsbpd_protlyr->
				send_msg, FIXED_SUPPLY);
			pr_info("\n=======================\n");
			pr_info("  SEND_COMMAND: SNK_CAP\n");
			pr_info("\n=======================\n");
			sii_usbpd_xmit_data_msg(pUsbpd_protlyr,
					SNKCAP, pUsbpd_protlyr->send_msg,
					1);
			pdev->next_state =
	...
	if (work) {
		work = 0;
		queue_work(pdev->ufp_work_queue, &pdev->pd_ufp_work);
	}
...
void wakeup_ufp_queue(struct sii70xx_drv_context *drv_context)
{
	...
	if (pdev->pd_connected) {
		if (queue_work(pdev->ufp_work_queue, &pdev->
			pd_ufp_work) == 0) {
			pr_info("%s():%d sm_work is already queue **\n",
				__func__, __LINE__);
		} else {
			pr_info("%s():%d sm_work queue OK**\n",
				__func__, __LINE__);
		}
	} else
