...
/*******************GLOBAL VARIABLE*********************/
extern s32 gt1x_request_irq(void);
...
void free_irq_gpio(void)
{
	int ret;
	struct irq_desc *desc = irq_to_desc(gt1x_i2c_client->irq);
	...
	if (ret < 0) {
		GTP_ERROR("before_irq_gpio: Failed to request GPIO:%d, ERRNO:%d", (s32) GTP_INT_PORT, ret);
		ret = -ENODEV;
	}
}

void use_irq_gpio(void)
{
	int ret;
	...
	if (ret < 0) {
		GTP_INFO("use_irq_gpio: GTP works in polling mode.");
	} else {
		GTP_INFO("use_irq_gpio: GTP works in interrupt mode.");
	}
}
