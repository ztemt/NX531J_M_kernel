...
#include <linux/completion.h>//add by luochangyang
...
enum {
	cyttsp_calibration_success,
	cyttsp_calibration_failed,
};
static int g_cali_result = cyttsp_calibration_failed;
static int upgrade_flag = 0;
/***ZTEMT END***/

/*ztemt flag of firmware updating*/
...
static void cyttsp5_calibrate_idacs(struct work_struct *calibration_work)
{
    ...
    int retry = 3;
    do{
        g_cali_result = cyttsp_calibration_failed;  //add by luochangyang 2013.09.12
        dev_dbg(dev, "%s calibration Start\n", __func__);
    	...
    	dev_err(dev, "%s: Calibration Done\n", __func__);
        g_cali_result = cyttsp_calibration_success;  //add by luochangyang 2013.09.12

    release:
    	cmd->release_exclusive(dev);
    exit:
    /*** ZTEMT Added by luochangyang, 2013/09/12 ***/
    	if (g_cali_result == cyttsp_calibration_success) {
    		dev_err(dev, "%s calibration success. End\n", __func__);
            retry = 0;
    	}else {
    		g_cali_result = cyttsp_calibration_failed;
    		retry--;
            mdelay(500);
    ...
    complete(&ld->cyttsp_cali_complete);
...
static int cyttsp5_load_app_(struct device *dev, const u8 *fw, int fw_size)
{
    ...
    upgrade_flag++;     //Added by luochangyang, 2014/02/18
...
exit:
	...
	if (wait_for_calibration_complete)
		wait_for_completion_timeout(&ld->calibration_complete, msecs_to_jiffies(7000));
...
static int ztemt_cyttsp5_check_firmware_version_builtin(struct device *dev)
{
...
static void _cyttsp5_firmware_force_upgrade(const struct firmware *fw,
		void *context)
{
	struct device *dev = context;
	struct cyttsp5_loader_data *data = cyttsp5_get_loader_data(dev);
		...
		void *context)
{
    ...
    upgrade = ztemt_cyttsp5_check_firmware_version_builtin(dev);
    dev_dbg(dev, "%s: upgrade= %d \n", __func__,upgrade);
...
/*** ZTEMT Modify by luochangyang, 2013/11/12 ***/
    ...
    if (ld->si == NULL) {
        dev_err(dev, "%s: data->si is NULL\n", __func__);

        retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
                CY_FW_FILE_NAME, dev, GFP_KERNEL, dev,
                _cyttsp5_firmware_force_upgrade);
    } else {
        retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
                filename, dev, GFP_KERNEL, dev,
                _cyttsp5_firmware_cont_builtin);
...
/*** ZTEMT Added by luochangyang, 2013/09/12 ***/
static ssize_t cyttsp5_fw_upgrade_flag_show(struct device *dev, struct 
device_attribute *attr, char *buf)
{	
	return snprintf(buf, PAGE_SIZE,"%d\n",upgrade_flag);
}
static DEVICE_ATTR(fw_upgrade_flag, 0444, cyttsp5_fw_upgrade_flag_show, NULL);

static ssize_t cyttsp5_manual_cali_show(struct device *dev, struct 
device_attribute *attr, char *buf)
{
    struct cyttsp5_loader_data *data = cyttsp5_get_loader_data(dev);
	//INIT_COMPLETION(data->cyttsp_cali_complete);
	reinit_completion(&data->cyttsp_cali_complete);
	g_cali_result = cyttsp_calibration_failed;
...
static ssize_t cyttsp5_manual_cali_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_loader_data *data = cyttsp5_get_loader_data(dev);
	schedule_work(&data->calibration_work);
...
static DEVICE_ATTR(manual_cali, 0664, cyttsp5_manual_cali_show, cyttsp5_manual_cali_store);
/***ZTEMT END***/
