...
static ssize_t cyttsp5_command_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    length = cyttsp5_ic_parse_input(dev, buf, size, dad->ic_buf,CY_MAX_PRBUF_SIZE);
    if (length <= 0) {
    ...
    if(dad->ic_buf[6] == 0x24) {
        element = get_unaligned_le16(&dad->ic_buf[9]);
        if(element > MAX_LEN) {
            dad->ic_buf[9] = MAX_LEN;
            dad->ic_buf[10] = 0;
            read = MAX_LEN;
            }
        else
        ...
        rc = cmd->nonhid_cmd->user_cmd(dev, 1, CY_MAX_PRBUF_SIZE,dad->response_buf, length, dad->ic_buf,&dad->response_length);
        ...
        while(element > 0) {
                         dad->ic_buf[7] = LOW_BYTE(read);
                         dad->ic_buf[8] = HI_BYTE(read);
                         if(element > MAX_LEN) {
                                dad->ic_buf[9] = MAX_LEN;
                                dad->ic_buf[10] = 0;
                         } else {
                                dad->ic_buf[9] = element;
                                dad->ic_buf[10] = 0;
                         }
                         rc = cmd->nonhid_cmd->user_cmd(dev,1,CY_MAX_PRBUF_SIZE,(dad->response_buf+num_read),length,dad->ic_buf,&dad->response_length);
                         if(rc){
                                dad->response_length = 0;
                                dev_err(dev, "%s: Failed to store command\n", __func__);
                                goto exit;
           ...
           if (rc) {
                  ...
                  dad->status = 1;
                  if(dad->ic_buf[6] == 0x24) {
                         dad->response_buf[7] =  LOW_BYTE(dad->response_length - 10); 
                         dad->response_buf[8] =  HI_BYTE(dad->response_length - 10);        
                  }
...
/*ZTEMT Added by luochangyang, 2014/02/21*/
/*return 0 IC detect success, return 1 failed*/
static ssize_t cyttsp5_ic_detect(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_device_access_data *dad = cyttsp5_get_device_access_data(dev);
    int retval = 0;
	...
	/*Open test*/
	cyttsp5_command_store(dev, attr, "0x04,0x00,0x05,0x00,0x2F,0x00,0x03", 35);
	msleep(50);
	if (dad->status != 1)
		retval = 1;

	for (i = 0; i < dad->response_length; i++)
		dev_info(dev, "%s: 0x%02X", __func__, dad->response_buf[i]);
	dev_info(dev, "\n");

	cyttsp5_command_store(dev, attr, 
	"0x04,0x00,0x07,0x00,0x2F,0x00,0x26,0x03,0x01", 45);
	msleep(100);
	if (dad->status != 1)
		retval = 1;
	if (dad->response_buf[5] == 0 && dad->response_buf[6] == 0 ) {
		dev_info(dev, "%s: IC open detect success.\n", __func__);
	} else {
		dev_err(dev, "%s: IC detect failed. Sensor is open.\n", __func__);
		retval = 1;
	}
	...
	cyttsp5_command_store(dev, attr, 
	"0x04,0x00,0x07,0x00,0x2F,0x00,0x26,0x04,0x00", 45);
	msleep(100);
	if (dad->status != 1)
		retval = 1;
	if (dad->response_buf[5] == 0 && dad->response_buf[6] == 0 ) {
		dev_info(dev, "%s: IC short detect success.\n", __func__);
	} else {
		dev_err(dev, "%s: IC detect failed. Sensor is short.\n", __func__);
		retval = 1;
	}
	...
	cyttsp5_command_store(dev, attr, "0x04,0x00,0x05,0x00,0x2F,0x00,0x04", 35);
	if (dad->status != 1)
		retval = 1;
...
static DEVICE_ATTR(ic_detect, 0444, cyttsp5_ic_detect, NULL);
/*ZTEMT END*/
...
{ 
    ...
    if(elem > MAX_ELEM) {
        rc = cyttsp5_ret_scan_data_cmd_(dev, elem_offset, MAX_ELEM, dad->heatmap.data_type, dad->ic_buf, &config,
                                &actual_read_len, NULL); 
        } else{ rc = cyttsp5_ret_scan_data_cmd_(dev, elem_offset, elem, dad->heatmap.data_type, dad->ic_buf, &config,
                                         &actual_read_len, NULL); 
        }
    ...
    elem_offset = actual_read_len;
    while (elem > 0) { 
        if(elem > MAX_ELEM) { rc = cyttsp5_ret_scan_data_cmd_(dev, elem_offset, MAX_ELEM, dad->heatmap.data_type, 
            NULL, &config,&actual_read_len, buf_offset); 
        } else { 
        rc = cyttsp5_ret_scan_data_cmd_(dev, elem_offset, elem, dad->heatmap.data_type, NULL, &config,
            &actual_read_len, buf_offset); 
        }
    if (rc < 0) 
        goto release_exclusive;
    if (!actual_read_len) break;
    length += actual_read_len * element_size;
    buf_offset = dad->ic_buf + length;
    elem -= actual_read_len;
    elem_offset += actual_read_len; }
...
put_runtime: 
    ...
    for (i = 0; i < length; i++) print_idx += scnprintf(buf_out + print_idx, TTHE_TUNER_MAX_BUF - print_idx,"%02X ", dad->ic_buf[i]); print_idx += 
    scnprintf(buf_out + print_idx, TTHE_TUNER_MAX_BUF - print_idx,":(%d bytes)\n", length); 
    rc = simple_read_from_buffer(buf, count, ppos, buf_out, print_idx); print_idx = rc;
release_mutex: 
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	/*ZTEMT Added by luochangyang, 2014/02/21*/
	rc = device_create_file(dev, &dev_attr_ic_detect);
	if (rc) {
		dev_err(dev, "%s: Error, could not create ic_detect\n",
				__func__);
		goto exit;
	}
	/*ZTEMT END*/
...
unregister_ic_detect:
    device_remove_file(dev, &dev_attr_ic_detect);
exit:
	return rc;
}
