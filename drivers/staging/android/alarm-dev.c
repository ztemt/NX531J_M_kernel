...
#define ANDROID_ALARM_WAKEUP_MASK ( \
	...
	ANDROID_ALARM_RTC_POWERON_WAKEUP_MASK | \
...
static int is_wakeup(enum android_alarm_type type)
{
	return (type == ANDROID_ALARM_RTC_WAKEUP ||
		...
		type == ANDROID_ALARM_RTC_POWERON_WAKEUP ||
...
static void alarm_clear(enum android_alarm_type alarm_type, struct timespec *ts)
{
	...
	if (alarm_type == ANDROID_ALARM_RTC_POWEROFF_WAKEUP ||
		alarm_type == ANDROID_ALARM_RTC_POWERON_WAKEUP)
		set_power_off_alarm(ts->tv_sec, 0);
							...
							struct timespec *ts)
{
	...
	if (alarm_type == ANDROID_ALARM_RTC_POWEROFF_WAKEUP ||
		alarm_type == ANDROID_ALARM_RTC_POWERON_WAKEUP)
		set_power_off_alarm(ts->tv_sec, 1);
	mutex_unlock(&alarm_mutex);
}
...
static int __init alarm_dev_init(void)
{
	...
	alarm_init(&alarms[ANDROID_ALARM_RTC_POWEROFF_WAKEUP].u.alrm,
			ALARM_REALTIME, devalarm_alarmhandler);
	alarm_init(&alarms[ANDROID_ALARM_RTC_POWERON_WAKEUP].u.alrm,
			ALARM_REALTIME, devalarm_alarmhandler);
